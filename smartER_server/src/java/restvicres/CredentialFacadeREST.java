...
public class CredentialFacadeREST extends AbstractFacade<Credential> {
    ...
    public List<Credential> findByRegistrationdate(@PathParam("registrationdate") String registrationdatestr) throws Exception{
        Date date=new SimpleDateFormat("yyyy-mm-dd").parse(registrationdatestr);   
...
@GET
    ...
    @Produces({"application/json"})
    public List<Credential> findByResid(@PathParam("resid") int resid) 
    {
        TypedQuery<Credential> qu = em.createQuery
